 ...
 * 관리자 대시보드 페이지
 *
 * @description 회원사 등록, API Key 관리, 만료 상태 모니터링
 * @route /admin
 * @access 관리자 권한 필요 - 현재는 권한 불필요
 */

"use client"

import { useState, useEffect } from "react"
import { Plus, Key, Users, Shield, Search, AlertTriangle, X, Ban, AlertCircle} from "lucide-react"
...
export default function AdminPage() {
    ...
    if (response.ok) {
      ...
      try {
        ...
        if (emailResponse.ok) {
          ...
          alert('API Key는 발급되었지만 이메일 전달에 실패했습니다. 수동으로 전달해주세요.')
        }
      } catch (emailError) {
        alert('API Key는 발급되었지만 이메일 전달에 실패했습니다.')
      }
      
      // 이메일 발송 성공/실패 여부와 관계없이 모달 표시
      setApiKeyModal({
        isOpen: true,
        apiKey: result.apiKey,
        mallName: newMall.mallName,
  ...
  const handleReissueMall = async () => {
    ...
    if (response.ok) {
      ...
      // API Key 이메일 발송 (이메일이 필수이므로 무조건 발송)
      try {
        const emailResponse = await fetch('/api/send-apikey', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
