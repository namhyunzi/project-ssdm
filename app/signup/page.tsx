...
export default function SignupPage() {
  ...
  const validateFullName = (name: string) => {
    if (!name.trim()) {
      setFieldError("fullName", "필수 입력 항목입니다.")
      return false
    }
    if (name.trim().length < 2) {
      setFieldError("fullName", "이름은 2자 이상 입력해주세요.")
      return false
    }
  ...
  const validatePhoneNumber = (phone: string) => {
    if (!phone.trim()) {
      setFieldError("phoneNumber", "필수 입력 항목입니다.")
      return false
    }
    // 숫자만 추출
    const numbersOnly = phone.replace(/[^0-9]/g, '')
    if (numbersOnly.length !== 11 || !numbersOnly.startsWith('010')) {
      setFieldError("phoneNumber", "올바른 휴대폰 번호를 입력해주세요. (010-0000-0000)")
      return false
    }
  ...
  const validateBirthDate = (date: string) => {
    ...
    if (!dateRegex.test(date)) {
      setFieldError("birthDate", "올바른 날짜 형식을 입력해주세요. (YYYY-MM-DD)")
      return false
    }
    ...
    if (inputDate > today) {
      setFieldError("birthDate", "미래 날짜는 입력할 수 없습니다.")
      return false
    }
  ...
  const handleTermsConsent = async (consentValues: {
    ...
    try {
      ...
      if (pendingGoogleUser) {
        // 1. 사용자 기본 정보 저장 (Google 사용자는 추가 정보를 나중에 입력)
        const userData = {
          email: pendingGoogleUser.email,
          displayName: pendingGoogleUser.displayName,
          fullName: pendingGoogleUser.displayName || "", // Google 이름을 기본값으로 사용
          phoneNumber: "", // 나중에 입력받음
          birthDate: null, // 나중에 입력받음
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        <Card className="w-full">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium">생년월일 <span className="text-gray-500">(선택)</span></label>
              <Input
                type="date"
                value={birthDate}
                onChange={(e) => {
          ...
          <Button 
            className="w-full bg-primary hover:bg-primary/90" 
                disabled={emailVerificationStep !== "verified" || !password || !passwordConfirm || !fullName || !phoneNumber || !termsAgreed || !privacyAgreed || isLoading}
            onClick={async () => {
              // 모든 필드 검증
              ...
              if (emailVerificationStep === "verified" && isPasswordValid && isPasswordConfirmValid && isFullNameValid && isPhoneNumberValid && isBirthDateValid) {
                const fullEmail = `${emailUsername}@${isDomainInputMode ? customDomain : emailDomain}`
