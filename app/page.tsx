...
export default function LoginPage() {
  const [loginType, setLoginType] = useState<"general" | "partner">("general")
  ...
  const handlePartnerLogin = async () => {
    if (!partnerId || !partnerPassword) {
      setToastMessage("파트너 ID와 비밀번호를 입력해주세요.")
      setShowToast(true)
    ...
    try {
      // 파트너 로그인 로직 (추후 구현)
      console.log('파트너 로그인:', { partnerId, partnerPassword })
      
      // 임시: 2초 후 성공 처리
      setTimeout(() => {
        setIsLoading(false)
        setToastMessage("파트너 로그인 성공!")
        setShowToast(true)
        setTimeout(() => {
          router.push('/dashboard') // 또는 파트너 전용 대시보드
        }, 1000)
      }, 2000)
      
    } catch (error) {
      setIsLoading(false)
      setToastMessage("파트너 로그인에 실패했습니다.")
      setShowToast(true)
      setTimeout(() => setShowToast(false), 3000)
    }
  }
    ...
    try {
      ...
      if (pendingGoogleUser) {
        ...
        const consentData = {
          ...
          marketingAgreed: consentValues.marketingAgreed, // 실제 동의 값 사용
      ...
      <Card className="w-full max-w-md">
          ...
          <div className="flex border-b border-border">
            ...
            <button
              ...
              className={`flex-1 py-3 px-4 text-sm font-medium transition-colors ${
                loginType === "partner"
                  ? "border-b-2 bg-blue-50"
                  : "text-muted-foreground hover:text-foreground"
              }`}
              style={loginType === "partner" ? { color: "#1e3a8a", borderColor: "#1e3a8a" } : {}}
            >
              파트너 로그인
            </button>
          </div>
          ...
          <CardHeader className="text-center space-y-4">
            ...
            <div className="text-center">
              <h1 className={`text-2xl font-bold ${loginType === "general" ? "text-primary" : ""}`} style={loginType === "partner" ? { color: "#1e3a8a" } : {}}>SSDM</h1>
              <p className="text-sm text-muted-foreground">
                {loginType === "general" ? "개인정보보호" : "파트너사 로그인"}
              </p>
            </div>
          </CardHeader>
